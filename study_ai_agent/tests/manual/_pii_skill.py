...
async def main():
    ...
    fake = FakeListChatModel(responses=["ack"] * 100)
    agent = create_agent(
        model=fake,
        tools=skill.tools,
        middleware=BASE_MIDDLEWARES,
        system_prompt=skill.system_prompt or "you are an assistant",
    )
    ...
    print("last:", str(last.content)[:80])
