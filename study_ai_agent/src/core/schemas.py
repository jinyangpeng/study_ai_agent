...
class PlanStep(BaseModel):
    ...
    expected_output: str = Field(default="", description="这一步成功的标志是什么。")
...
class Plan(BaseModel):
    ...
    rationale: str = Field(default="", description="为什么这个方案是对的。")
...
class Review(BaseModel):
    ...
    issues: list[str] = Field(default_factory=list, description="execute 输出里发现的具体问题。")
    suggestions: list[str] = Field(default_factory=list, description="下一轮应该做的具体修改。")
...
class Critique(BaseModel):
    ...
    * :class:`Critique` 是 Reflection 架构里"对照原始问题评价当前 draft
    ...
    verdict: Literal["approve", "revise"] = Field(..., description="'approve' 表示发布，'revise' 表示回 refine 重写。")
    issues: list[str] = Field(default_factory=list, description="当前 draft 里的具体问题。")
    suggestions: list[str] = Field(default_factory=list, description="下一轮 refine 应该做的具体修改。")
